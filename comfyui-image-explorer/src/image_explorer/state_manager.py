...
class StateManager:
    ...
    def _load(self):
        ...
        if not self.state_file.exists():
            logger.info(f"状態ファイルが存在しません。新規作成します: {self.state_file}")
            self._used_axes = set()
            return
        ...
        try:
            ...
            if version != "1.0":
                logger.warning(f"未知のバージョン: {version}。互換性の問題が発生する可能性があります")
            
            self._used_axes = set(data.get("used_axes", []))
            logger.info(f"状態ファイルを読み込みました: {len(self._used_axes)} 個の軸が使用済み")
...
def print_progress(state_manager: StateManager, all_axis_names: List[str]):
    ...
    print("=" * 50)
    print(f"探索進捗: {progress['used']}/{progress['total']} ({progress['percentage']:.1f}%)")
    print("=" * 50)
