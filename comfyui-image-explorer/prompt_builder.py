...
class PromptBuilder:
    ...
    def __init__(
        ...
        randomize_non_target: bool = True
    ):
        """
        Args:
...
class ParameterCombinationGenerator:
    ...
    def generate_combinations(
        ...
        # サンプラーの組み合わせ
        sampler_combos = list(itertools.product(
...
def example_full_generation():
    ...
    builder = PromptBuilder(
        prompt_template,
        randomize_non_target=execution_config.randomize_non_target
    )
    ...
    for i, params in enumerate(generator.generate_combinations(target_axis_name)):
        ...
        print(f"  サンプラー: {params.sampler.sampler_name} (steps={params.sampler.steps}, cfg={params.sampler.cfg})")
