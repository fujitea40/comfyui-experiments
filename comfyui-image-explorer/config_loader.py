...
def load_weighted_prompt(data: Union[str, Dict[str, Any]]) -> Union[str, WeightedPrompt]:
    """
    文字列または辞書からプロンプトを生成
...
def load_prompt_axis(data: Dict[str, Any]) -> PromptAxis:
    ...
    return PromptAxis(
        name=data["name"],
        choices=[load_weighted_prompt(c) for c in data["choices"]]
    )
...
class ConfigLoader:
    ...
    def __init__(self, config_path: Path):
        ...
        if suffix in [".yaml", ".yml"]:
            ...
            raise ValueError(f"未対応のファイル形式: {suffix}（.yaml, .yml, .json のみ対応）")
    ...
    def load_prompt_template(self) -> PromptTemplate:
        ...
        logger.info(f"プロンプトテンプレートを読み込みました（{len(template.axes)} 個の軸）")
        return template
    ...
    def load_sampler_choices(self) -> Dict[str, List]:
        ...
        logger.info(f"サンプラー選択肢を読み込みました")
        return choices
